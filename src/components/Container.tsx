  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:site_name " content="Ced Portfolio" />
        <meta property="og:description" content={meta.description} />
        <meta property="og:title" content={meta.title} />
        <meta property="og:image" content={meta.image} />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:site" content="@cedportfolio" />
      ...
      >
        ...
        <Link href="/">
          <span className="text-lg font-semibold clash-grotesk">Ced Portfolio</span>
        </Link>
        ...
        <AnimatePresence key="menu">
          {isOpen && (
            <motion.div
              className="fixed right-0 top-0 z-40 flex h-screen w-full flex-col justify-start overflow-y-hidden menu-background"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
            ...
            >
              <div className="flex h-20 max-h-20 min-h-[60px] w-full items-center justify-between border-b pl-[22px] pr-1">
                <span className="text-base font-medium lowercase text-menu-text-light dark:text-menu-text-dark text-muted-foreground">Menu</span>
                <button
                  onClick={() => setIsOpen(!isOpen)}
              ...
              <div className="flex h-full flex-col items-start justify-between overflow-y-auto text-muted-foreground">
                <ul className="flex min-h-fit w-full flex-col items-start space-y-6 px-[22px] py-[58px] text-muted-foreground">
                  {navLinks.map((link, i) => (
                ...
                <div className="flex min-h-fit w-full flex-col space-y-8 px-[22px] py-10">
                  <span className="text-sm text-muted-foreground">
                    © {new Date().getFullYear()} Cedric Bides. All rights reserved.
                  </span>
                </div>
              </div>
            </motion.div>
      ...
      <main className={cn("container mx-auto px-4 sm:px-6 lg:px-8", props.className)}>
        {children}
      </main>
      <Footer />
    </>
